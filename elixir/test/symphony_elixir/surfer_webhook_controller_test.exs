defmodule SymphonyElixir.SurferWebhookControllerTest do
  ...
  test "Linear webhook rejects non-AgentSessionEvent payloads without dispatching" do
    previous_secret = System.get_env("LINEAR_WEBHOOK_SECRET")
    on_exit(fn -> restore_env("LINEAR_WEBHOOK_SECRET", previous_secret) end)
    System.put_env("LINEAR_WEBHOOK_SECRET", "secret")
    ...
    body =
      Jason.encode!(%{
        webhookTimestamp: System.system_time(:millisecond),
        webhookId: "webhook-unsupported-type",
        type: "Issue",
        action: "created",
        agentSession: %{
          id: "session-unsupported-type",
          issue: %{id: "issue-unsupported-type", identifier: "ENG-1", title: "Fix", state: %{name: "Todo"}}
        }
      })
    ...
    assert json_response(conn, 400)["error"]["code"] == "unsupported_linear_event_type"
    refute_receive {:unexpected_started, _session_id, _run_id}, 100
    refute_receive {:unexpected_dispatch, _run_id}, 100
  end

  test "Linear webhook rejects missing event type without dispatching" do
    previous_secret = System.get_env("LINEAR_WEBHOOK_SECRET")
    on_exit(fn -> restore_env("LINEAR_WEBHOOK_SECRET", previous_secret) end)
    System.put_env("LINEAR_WEBHOOK_SECRET", "secret")
    ...
    body =
      Jason.encode!(%{
        webhookTimestamp: System.system_time(:millisecond),
        webhookId: "webhook-missing-type",
        action: "created",
        agentSession: %{
          id: "session-missing-type",
          issue: %{id: "issue-missing-type", identifier: "ENG-1", title: "Fix", state: %{name: "Todo"}}
        }
      })
    ...
    assert json_response(conn, 400)["error"]["code"] == "missing_linear_event_type"
    refute_receive {:unexpected_started, _session_id, _run_id}, 100
    refute_receive {:unexpected_dispatch, _run_id}, 100
  end

  test "Linear webhook rejects prompted sessions without agent activity id before dispatch" do
    previous_secret = System.get_env("LINEAR_WEBHOOK_SECRET")
    on_exit(fn -> restore_env("LINEAR_WEBHOOK_SECRET", previous_secret) end)
    System.put_env("LINEAR_WEBHOOK_SECRET", "secret")
    ...
    body =
      Jason.encode!(%{
        webhookTimestamp: System.system_time(:millisecond),
        webhookId: "webhook-missing-prompt-activity",
        type: "AgentSessionEvent",
        action: "prompted",
        agentSession: %{
          id: "session-missing-prompt-activity",
          issue: %{id: "issue-missing-prompt-activity", identifier: "ENG-1", title: "Fix", state: %{name: "Todo"}}
        }
      })
    ...
    assert json_response(conn, 400)["error"]["code"] == "missing_linear_agent_activity_id"
    refute_receive {:unexpected_started, _session_id, _run_id}, 100
    refute_receive {:unexpected_dispatch, _run_id}, 100
  end
  ...
  test "Discord interaction response retry window is anchored to interaction receipt" do
    {public_key, private_key} = :crypto.generate_key(:eddsa, :ed25519)
    previous_public_key = System.get_env("DISCORD_PUBLIC_KEY")
    previous_bot_token = System.get_env("DISCORD_BOT_TOKEN")
    ...
    db_path = Path.join(System.tmp_dir!(), "surfer-discord-followup-deadline-#{System.unique_integer([:positive])}.sqlite3")
    File.rm_rf(db_path)
    on_exit(fn -> File.rm_rf(db_path) end)
    ...
    Application.put_env(:symphony_elixir, :surfer_discord_interaction_retry_backoff_ms, 10)
    Application.put_env(:symphony_elixir, :surfer_discord_interaction_retry_window_ms, 25)
    ...
    command_body =
      Jason.encode!(%{
        id: "interaction-followup-deadline-1",
        application_id: "app-1",
        token: "interaction-token-secret",
        type: 2,
        guild_id: "guild-1",
        channel_id: "channel-1",
        member: %{user: %{id: "followup-deadline-user-1"}},
        data: %{
          name: "surfer",
          options: [%{name: "ask", type: 1, options: [%{name: "prompt", type: 3, value: "where is routing handled?"}]}]
        }
    ...
    assert_receive {:interaction_response_attempt, "app-1", "interaction-token-secret", body}
    refute_receive {:interaction_response_attempt, "app-1", "interaction-token-secret", ^body}, 30
    assert_receive {:channel_fallback, "channel-1", ^body}
  ...
  test "Discord lifecycle interaction without run id fails before claim or dispatch" do
    ...
    db_path = Path.join(System.tmp_dir!(), "surfer-discord-missing-run-id-#{System.unique_integer([:positive])}.sqlite3")
    File.rm_rf(db_path)
    on_exit(fn -> File.rm_rf(db_path) end)
    ...
    body =
      Jason.encode!(%{
        id: "interaction-missing-lifecycle-run-id",
        application_id: "app-1",
        token: "missing-run-id-token",
        type: 2,
        guild_id: "guild-1",
        channel_id: "channel-1",
        member: %{user: %{id: "user-1"}},
    ...
    assert json_response(conn, 400)["error"]["code"] == "missing_discord_lifecycle_run_id"
    refute_receive {:unexpected_dispatch, _run_id}, 100
    assert {:error, :not_found} = RunLedger.lookup_idempotency_key(db_path, "discord_interaction:interaction-missing-lifecycle-run-id:lifecycle_control")
  end

  test "Discord interaction with unsupported subcommand fails before claim or dispatch" do
    {public_key, private_key} = :crypto.generate_key(:eddsa, :ed25519)
    previous_public_key = System.get_env("DISCORD_PUBLIC_KEY")
    ...
    db_path = Path.join(System.tmp_dir!(), "surfer-discord-unsupported-subcommand-#{System.unique_integer([:positive])}.sqlite3")
    File.rm_rf(db_path)
    on_exit(fn -> File.rm_rf(db_path) end)
    ...
    body =
      Jason.encode!(%{
        ...
        application_id: "app-1",
        token: "unsupported-subcommand-token",
        type: 2,
        guild_id: "guild-1",
        channel_id: "channel-1",
        member: %{user: %{id: "user-1"}},
    ...
    assert json_response(conn, 400)["error"]["code"] == "unsupported_discord_subcommand"
    refute_receive {:unexpected_dispatch, _run_id}, 100
